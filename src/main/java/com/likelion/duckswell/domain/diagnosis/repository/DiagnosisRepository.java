...
public interface DiagnosisRepository extends JpaRepository<Diagnosis, Long> {
    ...
    /**
     * courseIds에 속한 루틴 중 점수 3개가 모두 채워진 진단만 골라, 루틴 날짜(같으면 routine id)
     * 내림차순으로 DB에서 직접 pageable 크기만큼 제한 조회한다 - 전체 이력을 메모리에 올려
     * 애플리케이션에서 필터링/정렬/제한하지 않기 위함. 필요한 점수 3개만 프로젝션한다(회복 배너의
     * "현재/직전 기록" 비교에 사용).
     */
    @Query("""
            ...
            ORDER BY r.routineDate DESC, r.id DESC
            """)
