...
public interface DashboardApi {
    ...
    @Operation(
            ...
            description = """
                    ...
                    redness/texture/blemish: "오늘" 날짜나 지금 코스로 범위를 좁히지 않고, 회원의
                    코스 전체(진행 중 + 종료된 코스 모두)를 통틀어 실제로 존재하는 가장 최근 진단
                    기록 2개를 기준으로 계산합니다 - 가장 최근 기록의 점수(current)와 그 직전
                    기록의 점수(previous)의 차이(delta, +는 악화·-는 개선)입니다. 새 기록이 생기면
                    그 기록이 current로, 기존 current였던 기록이 previous로 밀려 올라갑니다. 새
                    기록이 없으면 있던 기록들끼리(예: 어제/그제, 혹은 직전 코스의 마지막 기록) 그대로
                    비교됩니다. 새 코스를 시작해도 직전 코스의 마지막 기록이 그대로 이어져서
                    previous/delta가 다시 0으로 리셋되지 않습니다. 유효한 기록이 하나도 없으면
                    current/previous/delta 모두 0이고, 기록이 1개뿐이면 previous/delta만 0으로 고정됩니다.
